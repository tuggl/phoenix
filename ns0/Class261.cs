using System;
namespace ns0
{
	internal sealed class Class261 : Interface0
	{
		public void imethod_0(Class16 class16_0, Class18 class18_0)
		{
			Class14 @class = Phoenix.smethod_15().method_9().method_19(class16_0.method_2().uint_3);
			if (@class != null)
			{
				Class33 class2 = @class.method_53(class16_0.method_2().uint_0);
				if (class2 != null)
				{
					class2.method_0();
					class2.int_15 = 0;
					GClass5 gClass = new GClass5(481u);
					gClass.method_9(class2.int_0);
					@class.method_59(gClass, null);
					if (class16_0.method_2().uint_6 == 8u)
					{
						Phoenix.smethod_15().method_16().method_1(8u, class16_0);
					}
				}
			}
		}
	}
}
